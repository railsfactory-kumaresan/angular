...
desc "insert_comments"

task :insert_comment , [:question_count,:sharing_count] => [:environment] do |t,params|
...
@user = 0
  ...
  #create Comment type Question
  @question_count.times do
    faker_hash_comment << {:category_type_id  => 1, :expiration_id => "1 Day", :question => "#{Faker::Lorem.words(5).join(" ")}?" , :video_type => 1, :embed_url => "", :include_text => 0, :question_type_id => 4, :thanks_response => 'Thanks for responding!', :user_id => @user.id ,:created_at => @created_date, :updated_at =>@created_date, :status => 'Active', :expired_at => Time.now + 30.days,:slug => SecureRandom.urlsafe_base64(nil, false)}
  end

  batch = []
  faker_hash_comment.each do |row|
  ...
  question_list.each do |i|   
    ...
    @providers.each do |r|
      ...
      @sharing_count.times do
        ...
        answer_hash << {:is_business_user => false,:question_type_id => i.question_type_id, :comments => Faker::Lorem.words(2).join(" "), :question_id => i.id, :provider => r  , :is_deactivated => false,  :created_at => @created_at, :updated_at => @created_at , :uuid => @id, :customers_info_id => @id  }
