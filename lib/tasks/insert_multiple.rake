desc "insert_multiple_answer"

task :insert_multiple_answer , [:question_count,:sharing_count] => [:environment] do |t,params|
  ...
  @question_count.times do
    faker_hash_multiple << {:category_type_id  => 2, :expiration_id => "1 Day", :question => "#{Faker::Lorem.words(5).join(" ")}?" , :video_type => 1, :embed_url => "", :include_text => 0, :question_type_id => 2, :thanks_response => 'Thanks for responding!', :user_id => @user.id ,:created_at => @created_date, :updated_at =>@created_date, :status => 'Active', :expired_at => Time.now + 30.days,:slug => SecureRandom.urlsafe_base64(nil, false)}
  end

  batch = []
  faker_hash_multiple.each do |row|
  ...
  question_list.each do |i|
    @option = ["option_#{Faker::Lorem.words(1).join(" ")}", "option_#{Faker::Lorem.words(1).join(" ")}"]
