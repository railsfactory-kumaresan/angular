desc :load_view_count
task :increment_view_count => [:environment]  do

cron_log = CronLog.where("log_type=?","QuestionViewLog").first
last_run_id = cron_log.last_run_id.nil? ? 0 : cron_log.last_run_id
log_records = QuestionViewLog.where("id > ?",last_run_id).order('id')
unless log_records.empty?
log_records.each do |question_log|
vrtype = "view"#
process_view(question_log,vrtype)# if question_log.is_processed?
...
private
def process_view(question_log,vrtype)
